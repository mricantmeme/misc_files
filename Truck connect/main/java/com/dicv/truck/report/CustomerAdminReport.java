...
public class CustomerAdminReport {

	private final Collection<UserDto> list = new ArrayList<>();

	public CustomerAdminReport(Collection<UserDto> c) {
		list.addAll(c);
	}
	...
	private DynamicReport getReport(Style headerStyle, Style detailTextStyle, Style detailNumStyle)
		...
		AbstractColumn lastName = createColumn("lastName", String.class, "Last Name", headerStyle, detailTextStyle,
				140);
		AbstractColumn companyName = createColumn("companyName", String.class, "Company Name", headerStyle,
				detailTextStyle, 140);
		AbstractColumn companyAddress = createColumn("companyAddress", String.class, "Company Address", headerStyle,
				detailTextStyle, 200);
		...
		report.setReportName("CustomerAdminReport").addColumn(firstName).addColumn(lastName)
				.addColumn(companyName).addColumn(companyAddress).addColumn(mobileNumber).addColumn(emailId)
				.addColumn(stateName).addColumn(cityName).addColumn(countryCode).setUseFullPageWidth(true);
